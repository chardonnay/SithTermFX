...
import com.sithtermfx.core.TerminalExecutorServiceManager;
import com.sithtermfx.core.TerminalMode;
import com.sithtermfx.core.TerminalStarter;
...
import com.sithtermfx.ui.model.DefaultTypeAheadTerminalModel;
...
/**
 * SithTermFX terminal widget with UI implemented in JavaFX.
 */
public class SithTermFxWidget implements TerminalSession, TerminalWidget, TerminalActionProvider {

    private static final Logger logger = LoggerFactory.getLogger(SithTermFxWidget.class);

    protected final TerminalPanel myTerminalPanel;

    protected final SithTerminal myTerminal;
...
//  private final PreConnectHandler myPreConnectHandler;
    ...
    public SithTermFxWidget(@NotNull SettingsProvider settingsProvider) {
        this(80, 24, settingsProvider);
    }

    public SithTermFxWidget(int columns, int lines, SettingsProvider settingsProvider) {
...
//    myTerminalPanel.addCustomKeyListener(myPreConnectHandler);
    ...
    protected @NotNull SithTerminal createTerminal(@NotNull TerminalDisplay display,
                @NotNull TerminalTextBuffer textBuffer, @NotNull StyleState initialStyleState) {
        return new SithTerminal(display, textBuffer, initialStyleState);
    }

    @SuppressWarnings({"removal", "DeprecatedIsStillUsed"})
    @Deprecated(forRemoval = true)
    private PreConnectHandler createPreConnectHandler(SithTerminal terminal) {
    ...
    @Override
    public SithTermFxWidget createTerminalSession(TtyConnector ttyConnector) {
            ...
            try {
                ...
                try {
                    for (TerminalWidgetListener listener : myListeners) {
                        listener.allSessionsClosed(SithTermFxWidget.this);
                    }
                } catch (Exception e) {
