...
All examples reference the public API documented in
[`docs/public-api.md`](public-api.md). A working reference implementation can
be found in the [korTTY](https://github.com/chardonnay/korTTY) SSH client.
...
2. [Quick Start — Minimal Terminal](#quick-start)
3. [Connecting a Real Shell (PTY)](#real-shell)
4. [Custom TtyConnector (SSH Example)](#custom-connector)
5. [Theming and Colors](#theming)
6. [Dynamic Font Size](#dynamic-font-size)
7. [Split Terminal](#split-terminal)
8. [Hyperlinks](#hyperlinks)
9. [Terminal Lifecycle](#lifecycle)
10. [Advanced Integration Patterns](#advanced)
...
## 2. Quick Start — Minimal Terminal <a name="quick-start"></a>
...
- `DefaultSettingsProvider` gives sensible defaults (Menlo/Consolas font,
  14pt, light background).
- Call `widget.start()` after `setTtyConnector()` to begin terminal emulation.
- Always call `widget.close()` on shutdown.

---

## 3. Connecting a Real Shell (PTY) <a name="real-shell"></a>

For a real interactive shell, use [Pty4J](https://github.com/JetBrains/pty4j)
...
## 4. Custom TtyConnector (SSH Example) <a name="custom-connector"></a>
...
[korTTY](https://github.com/chardonnay/korTTY) implementation using
Apache MINA SSHD:
...
## 5. Theming and Colors <a name="theming"></a>

### Dark Theme

Override `getDefaultForeground()` and `getDefaultBackground()` in a custom
settings provider:
...
Access the terminal's `StyleState` to change foreground/background colors
...
## 6. Dynamic Font Size <a name="dynamic-font-size"></a>

Use `DynamicFontSizeSettingsProvider` to enable runtime font resizing
...
## 7. Split Terminal <a name="split-terminal"></a>

`TerminalSplitPane` manages nested horizontal/vertical splits with
independent sessions, context menus, broadcast mode, and drag-and-drop
reordering.
...
## 8. Hyperlinks <a name="hyperlinks"></a>
...
## 9. Terminal Lifecycle <a name="lifecycle"></a>
...
## 10. Advanced Integration Patterns <a name="advanced"></a>

These patterns are extracted from the
[korTTY](https://github.com/chardonnay/korTTY) SSH client, which is a
...
│                                                 │
│  ┌───────────────┐   ┌───────────────────────┐  │
│  │SettingsProvider│   │   SplitConnectorFactory│  │
│  │(colors, fonts) │   │   (creates sessions)  │  │
│  └───────┬───────┘   └───────────┬───────────┘  │
│          │                       │               │
│          ▼                       ▼               │
│  ┌───────────────────────────────────────────┐  │
│  │           TerminalSplitPane               │  │
│  │  ┌─────────────────┐ ┌─────────────────┐ │  │
│  │  │ SithTermFxWidget │ │ SithTermFxWidget │ │  │
...
│              ▼                   ▼               │
│  ┌───────────────────┐ ┌───────────────────┐    │
│  │  TtyConnector #1  │ │  TtyConnector #2  │    │
│  │  (PTY / SSH / …)  │ │  (PTY / SSH / …)  │    │
│  └───────────────────┘ └───────────────────┘    │
└─────────────────────────────────────────────────┘

sithtermfx-core:  TtyConnector, TerminalColor, TextStyle,
                  SithTerminal, CursorShape, TermSize

sithtermfx-ui:    SithTermFxWidget, TerminalPanel,
                  TerminalSplitPane, SplitConnectorFactory,
                  SplitRequest, DefaultSettingsProvider,
                  DynamicFontSizeSettingsProvider,
                  DefaultHyperlinkFilter
```
...
Source code:
- [`BasicTerminalShellExample.java`](../sithtermfx-app/src/main/java/com/sithtermfx/app/example/BasicTerminalShellExample.java)
- [`SplitTerminalShellExample.java`](../sithtermfx-app/src/main/java/com/sithtermfx/app/example/SplitTerminalShellExample.java)
- [`BasicTerminalExample.java`](../sithtermfx-app/src/main/java/com/sithtermfx/app/example/BasicTerminalExample.java)
